...
// Funcao que recebe um grafo, vertice inicial e vetor de visitados, percorrendo o grafo com DFS
void dfs(int grafo[][TAM_MATRIZ], int vertice, int* visitados) {
    ...
    for(w = 0; w < TAM_MATRIZ; w++) {
        if(visitados[w] == 0 && grafo[vertice][w] == 1) {
            dfs(grafo, w, visitados);
        }
    }
}

// Funcao que recebe um grafo, vertice inicial, vetor de visitados e fila de vértices, percorrendo o grafo com BFS
void bfs(int grafo[][TAM_MATRIZ], int vertice, int* visitados, TipoDFila *PtDFila) {
    int w;

    visitados[vertice] = 1;

    InserirFila(&PtDFila, vertice);

    while(!VaziaFila(PtDFila)) {
        RemoverFila(&PtDFila, &vertice);

        printf("%d ", vertice + 1);

        for(w = 0; w < TAM_MATRIZ; w++) {
            if(visitados[w] == 0 && grafo[vertice][w] == 1) {
                InserirFila(&PtDFila, w);
                visitados[w] = 1;
            }
        }
    }
